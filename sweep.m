function res = sweep(velocity_range, timespan)
    ...
    for i=1:length(velocity_range)
        ...
        for j=(length(thetas) - 1):-1:1
           ...
           if (sign_1 ~= theta_direction) && sign_1 ~= 0 && (sign_2 ~= theta_direction) && sign_2 ~= 0
               fprintf("BREAKING! s1: %d s2: %d j: %d theta_direction: %d prev_theta: %d theta: %d\n", sign_1, sign_2, j, theta_direction, prev_theta, thetas(j)); 
               break
           end
        ...
        polarplot(thetas_in_last_orbit, ones(length(thetas_in_last_orbit), 1));
        pax = gca;
